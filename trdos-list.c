...
void unpack_disk_info(unsigned char *buffer, disk_info *info) {
...
void unpack_file_info(unsigned char *buffer, file_info *info) {
...
int read_disk_info(FILE *fp, disk_info *info) {
    ...
    unpack_disk_info(buffer, info);
    free(buffer);

    return 0;
}

int read_file_info(FILE *fp, file_info *info) {
    unsigned char *buffer;
...
int list(char *filename) {
    ...
    result = read_disk_info(fp, &disk_info);
    if (result != 0) {
        return result;
    }
    ...
    for (unsigned char i = 0; i < disk_info.num_total_files; i++) {
        result = read_file_info(fp, &file_info);
        if (result != 0) {
            return result;
        }
